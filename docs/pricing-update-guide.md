...
|------|---------|---------|
| 2026-09-23 | 0.7.7 | Waiver retirement: the 5 providers whose verification waivers expired 2026-10-01 re-read from sources a script can fetch. **Amazon Nova** — AWS Price List API `offers/v1.0/aws/AmazonBedrock/current/us-east-1/index.json` (per-1K meters × 1000): all 5 rows unchanged, cache-read rates added, now `Official`; Nova 2 Lite's row is the global cross-region price (no in-region us-east-1 endpoint); Nova 2 Pro / Nova 2 Omni have meters but no Bedrock model card (not confirmed GA) — deferred. **Azure** — Retail Prices API (`serviceName eq 'Foundry Models'`, Global meters; `pp` = priority, `Fl` = flex, ignored): existing rows unchanged except gpt-35-turbo (regional only, 0.55/1.65); gpt-4 regional only; +GPT-5 mini/nano/pro/5.1/5.2; GPT-5.4/5.5/5.6/6 short/long-context meters deferred (the threshold is not in the docs); gpt-6-sol/luna have no meter yet. **Qwen** — alibabacloud.com/help/en/model-studio/model-pricing (Singapore, USD): both rows were wrong (see CHANGELOG); cache is a multiplier (implicit ~20%, explicit write 125% / hit 10%) — not recorded as a fixed rate. **Cohere** — cohere.com/pricing embedded JSON: Command R (08-2024) $0.15/$0.60, R7B unchanged, Command A no longer listed (aggregators only → `ThirdParty`); deprecations 2025-09-15 per docs.cohere.com/docs/deprecations. **Meta Llama** — Meta's developer site redirects to a Model API that lists no Llama models; DeepInfra is the reference (Maverick 0.20/0.80, Scout 0.10/0.30). The staleness script's waiver list is empty after this release. |
| 2026-09-23 | 0.7.6 | Added Claude Opus 5.5 ($4 / $20, cache write $5, cache read $0.20 — 0.05x input; 1M / 128K; thinking always on); its id previously fell through to the `claude-opus-5` contains alias and was priced as Opus 5. Sonnet 5 re-checked ($2 / $10 is now standard). Same pass refreshed the four providers past the 30-day cadence plus OpenAI: **OpenAI** +GPT-6 Sol / Luna (272K band: 2x input/cache, 1.5x output), cache-write rates for GPT-6 and GPT-5.6, no price changes to existing rows (codex, o1-mini and deep-research rows are no longer on the page — kept, unverified). **xAI** +Grok 4.7; tier cache-read rates; legacy ids that the API now answers as Grok 4.3 / Grok Build 0.1 (`GET /v1/language-models/<id>`) moved to those rows as aliases; two not-found `-thinking` ids removed. **DeepSeek** V4 Flash → V4.1 Flash (`deepseek-flash`, 0.15 / 0.60, effective 2026-09-10; legacy name billed at Flash); V4 Pro unchanged (the announced 09-14 routing to Flash was withdrawn); `deepseek-chat` / `-reasoner` discontinued 2026-07-24 — rows kept for past usage. **Mistral** +Ministral 3 3B/8B/14B, +GLM-5.3 (hosted; the page's cached rate is not recorded because Mistral does not state its caching mode); context windows corrected; Devstral, Magistral and Pixtral retired 2026-03..07 — rows kept. **Perplexity** Sonar API is supported only until 2026-09-27 (replaced by the Agent API); `sonar-reasoning` deprecated 2025-12-15; Sonar Deep Research context 128K. Deferred: Agent API models (`perplexity/sonar` 0.25/2.50, `perplexity/glm-5.3`, `perplexity/kimi-k3`, …) — prices captured, context windows not published → next refresh. Sources: platform.claude.com, developers.openai.com + `/v1/models`, docs.x.ai + `/v1/language-models`, api-docs.deepseek.com, mistral.ai/pricing/api + docs.mistral.ai, docs.perplexity.ai |
